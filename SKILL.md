...
Plate URL. `edit --input` appends one flat item in the shared format, whatever
produced it, carrying all four core nutrients and a positive `grams` naming the
weight they describe, which for per-100 g figures is `100`. An item stating no
